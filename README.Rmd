...
for (i in 1:4){
  image(matrix(unlist(x[[i]]), M, M),
        asp = 1, col = c("black", "lightgray"),
        axes = FALSE)
  title(main=paste("Time =", at[i]))}
```

```{r, fig.width = 5, fig.height = 5, fig.show = 'animate'}
# this illustrates using a custom observer to animate
# the animation is generated by the knitr package
# the output only shows in the generated html
f = function(x, t) image(matrix(x, M, M), asp = 1, axes = F,
                        col = grey(seq(0.1, 1, len = 100)), main = round(t))
compile_sys("bistab", bistable, sys_dim = M * M,
             pars = c(D = 0.1, a = 1.0, b = 1/2),
